...
import { CATEGORIES_CONFIG, KEY_POINT_CONFIG, QUESTION_DATABASE } from '../constants';
...
const KeyPointStats: React.FC<{ title: string; data: { name: string; score: number }[], color: string }> = ({ title, data, color }) => (
            ...
            {data.length > 0 ? data.map(kp => (
                <div key={kp.name}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                        <span style={{ fontSize: '0.9rem' }}>{kp.name}</span>
...
const StatisticsScreen: React.FC<StatisticsScreenProps> = ({ user, gameState, onBack }) => {
    ...
    const totalQuestionsAnswered = gameState.currentQuestionIndex;
    const averageOverallScore = totalQuestionsAnswered > 0 ? gameState.rating / totalQuestionsAnswered : 0;
            ...
            <main style={mainContentStyle}>
                <div style={summaryGridStyle}>
                    <div style={styles.statsSummaryCard}>
                        ...
                        <div style={{color: 'var(--text-secondary)', fontSize: '0.8rem'}}>Общий рейтинг</div>
                    </div>
                    <div style={styles.statsSummaryCard}>
                        <div style={{...summaryCardValueStyle, color: 'var(--secondary-color)'}}>{totalQuestionsAnswered}</div>
                        <div style={{color: 'var(--text-secondary)', fontSize: '0.8rem'}}>Отвечено</div>
                    </div>
                     <div style={styles.statsSummaryCard}>
                        <div style={{...summaryCardValueStyle, color: '#00e5ff'}}>{QUESTION_DATABASE.length}</div>
                        <div style={{color: 'var(--text-secondary)', fontSize: '0.8rem'}}>Всего вопросов</div>
                    </div>
                    <div style={styles.statsSummaryCard}>
                        <div style={{...summaryCardValueStyle, color: '#ffc400'}}>{averageOverallScore.toFixed(1)}</div>
                        <div style={{color: 'var(--text-secondary)', fontSize: '0.8rem'}}>Средняя оценка</div>
                    </div>
                </div>
                ...
                <div style={styles.chartCard}>
                    ...
                    <div style={styles.chartContainer}>
                        {Object.entries(CATEGORIES_CONFIG).map(([key, { name, color }]) => {
                            ...
                            return (
                                <div key={key} style={styles.chartBarWrapper}>
                                    <div style={{ color: 'var(--text-primary)', marginBottom: '0.5rem', fontWeight: 600 }}>{Math.round(value)}%</div>
                                    <div style={{...styles.chartBar, height: `${value}%`, backgroundColor: color}}></div>
                                    <div style={chartLabelStyle}>{name}</div>
                                </div>
                            )
                        })}
                    </div>
