...
• При подозрении на проблемы с рандомизацией`
    ...
    { 
        ...
        modules: ['1.1'],
        referenceAnswer: "Принцип 'одно изменение за раз' критически важен для установления причинно-следственной связи. Если мы меняем кнопку И цвет И текст одновременно, и видим рост конверсии, мы не знаем, что именно сработало. Возможно, кнопка улучшила результат, а цвет ухудшил, но суммарно мы видим плюс. Или наоборот. Это делает невозможным обучение и масштабирование находок. В BigTech это железное правило: Google, Facebook, Amazon тестируют каждое изменение отдельно, чтобы строить системное понимание того, что работает."
    },
    { 
        id: 22, 
        difficulty: 3, 
    ...
    // ------------------------------------------------------------------------
    {
        ...
        modules: ['1.1'],
        referenceAnswer: "Общие паттерны успешных A/B тестов: 1) Тестирование на огромном масштабе (миллионы пользователей = высокая чувствительность к малым изменениям), 2) Четкие бизнес-метрики (revenue, engagement, retention - не абстрактные 'улучшения'), 3) Культура экспериментирования (каждое изменение тестируется, не полагаются на мнения), 4) Долгосрочное мышление (небольшие улучшения × масштаб × время = огромный эффект). Google заработала $200M не потому что тест был сложным, а потому что имела инфраструктуру для быстрого тестирования и миллиарды показов для монетизации даже 0.1% улучшения."
    },
    {
        id: 112,
        difficulty: 2,
        ...
        modules: ['1.1'],
        referenceAnswer: "A/B тесты критичны, когда: 1) Изменение влияет на ключевые метрики (revenue, retention, engagement), 2) Интуиция может быть ошибочна (то, что хорошо в user research, может не работать на практике - люди говорят одно, делают другое), 3) Эффект может быть небольшим, но важным (2-3% улучшение конверсии = миллионы $), 4) Откат дорог (потеря доверия пользователей, репутации). Можно обойтись без A/B: критические баги (нужен быстрый фикс), очевидные улучшения (исправление опечаток), малый трафик (недостаточно данных), этические/legal ограничения. Но в большинстве случаев тестирование - это страховка от дорогих ошибок и способ систематического улучшения продукта."
    },
    {
        id: 113,
        difficulty: 3,
        ...
        keyPoints: ['causal-inference', 'randomization', 'experiment-types'],
        modules: ['1.1'],
        referenceAnswer: "Before/after НЕ A/B тест, потому что отсутствует рандомизированная контрольная группа. Факторы, искажающие выводы: 1) Сезонность (праздники, выходные, начало месяца), 2) Внешние события (конкуренты запустили акцию, в новостях что-то произошло), 3) Естественный рост продукта (органический рост метрик со временем), 4) Selection bias (новая фича могла привлечь другой тип пользователей), 5) Regression to the mean (если до запуска был провал, после естественно будет восстановление). Before/after допустим: для очень крупных изменений с очевидным эффектом, когда нет возможности рандомизации (инфраструктурные изменения, регуляторные требования). A/B тест обязателен: когда нужна точная оценка эффекта, когда изменение влияет на критичные метрики, когда эффект может быть небольшим."
    },
